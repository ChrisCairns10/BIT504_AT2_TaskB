...
public class Board {
	...
	public Board() {
	 ...
	 // Cells array is initialised using ROWS and COLS constants
	...
	public boolean isDraw() {
		...
		//Game is checked to see if it has ended in a draw. 'True' returned if there are no empty positions left.
		boolean drawn = true;

		for(int row = 0; row < GameMain.ROWS; ++row) {
			for (int col = 0; col < GameMain.COLS; ++col) {
				if (cells[row][col].content == Player.Empty) //Check if any of the cells content in the board grid are Player.Empty 
				drawn = false; // 'False' returned if not a draw
			}
		}
		return drawn;	
	}
	...
	public boolean hasWon(Player thePlayer, int playerRow, int playerCol) {
		...
		// Check if the player has the diagonal in the other direction
